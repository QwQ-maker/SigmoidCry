...
# 运行显示/调试开关
# =====================================================================
# 建议调试时先用 5~20；确认无误后再改回 200
# N_SAMPLES = 20

VERBOSE = True              # True: 打印每张图片的每个阶段；False: 只打印总体进度
PRINT_EVERY = 1             # 每多少张图片打印一次汇总；调试建议 1，正式跑可改 10
SHOW_SYSTEM_STATUS = True   # 显示 CPU/内存占用；需要 psutil，没安装也不会报错
...
def log(msg, flush=True):
    print(f"[{now_str()}] {msg}", flush=flush)


def system_status():
    if not SHOW_SYSTEM_STATUS or psutil is None:
        return ""
    try:
...
class StageTimer:
    def __init__(self, name, timings, key, verbose=True):
        self.name = name
    ...
    def __enter__(self):
        self.t0 = time.time()
        if self.verbose:
            log(f"    -> {self.name} 开始{system_status()}")
        return self

    def __exit__(self, exc_type, exc, tb):
        dt = time.time() - self.t0
        self.timings[self.key] = dt
        if self.verbose:
            if exc_type is None:
                log(f"    <- {self.name} 完成，用时 {dt:.2f}s{system_status()}")
            else:
                log(f"    !! {self.name} 出错，已运行 {dt:.2f}s")
        return False
...
def run_experiment(activation_type, ctx, images, labels,
    ...
    if tqdm is not None and not VERBOSE:
        iterator = enumerate(tqdm(images, desc=f"{activation_type.upper()} 密文推理", unit="张"))

    for i, img in iterator:
        log(f"\n[{activation_type.upper()}] 开始第 {i+1}/{len(images)} 张图像{system_status()}")
        t0 = time.time()
        logits, timings = encrypted_forward(
            ctx, img, weights, coeffs, im2col_idx, out_h, out_w, verbose=VERBOSE)
        ...
        if ((i + 1) % PRINT_EVERY == 0) or i == 0 or (i + 1) == len(images):
            slowest_stage = max(
                [k for k in timings.keys() if k != "total"],
                key=lambda k: timings[k]
            )
            log(f"  进度 {i+1:3d}/{len(images)} | 本张 {timings['total']:.2f}s | "
                f"已耗 {elapsed:.0f}s | 预计剩余 {eta:.0f}s | "
                f"最慢阶段 {slowest_stage}={timings[slowest_stage]:.2f}s")
            log(f"  分类: 真={FASHION_LABELS[labels[i]]} | "
                f"明={FASHION_LABELS[plain_preds[i]]} | "
                f"密={FASHION_LABELS[cipher_preds[i]]}")
            log("  阶段耗时: " + ", ".join(
                f"{k}={timings[k]:.2f}s"
                for k in ['encrypt', 'conv1', 'act1', 'pool', 'repack',
...
def main():
    log(f"加载 {N_SAMPLES} 张 Fashion-MNIST 测试图像 ...")
    images, labels = load_test_data(N_SAMPLES)

    im2col_idx, out_h, out_w = im2col_indices(28, 28, 5, 5, 2)
    log(f"卷积输出尺寸: {out_h}x{out_w} = {out_h*out_w} 窗口/通道")

    log("初始化 CKKS 上下文 (Poly-3) ...")
    ...
    # 小样本调试版：只跑 Poly-3，跳过 Poly-5，速度更快。
    # 确认能跑通后，再使用完整版脚本运行 Poly-3 + Poly-5。
    log("\n[小样本调试版] 已跳过 Poly-5，以减少等待时间。")

    # 小样本版只保存 Poly-3 的基础结果，避免调用需要 r5 的绘图函数。
    df = pd.DataFrame([{
        '激活函数': 'Poly-3',
        '明文准确率(%)': f"{r3['plain_acc']:.2f}",
        '密文准确率(%)': f"{r3['cipher_acc']:.2f}",
    ...
    df.to_csv(os.path.join(OUT_DIR, 'debug_poly3_summary.csv'),
              index=False, encoding='utf-8-sig')
    ...
    df_t.to_csv(os.path.join(OUT_DIR, 'debug_poly3_time_breakdown.csv'),
                index=False, encoding='utf-8-sig')

    log("\n[完成] 小样本 Poly-3 调试实验已完成。")
    log(f"  输出目录: {OUT_DIR}")
    log(f"  已保存: {os.path.join(OUT_DIR, 'debug_poly3_summary.csv')}")
    log(f"  已保存: {os.path.join(OUT_DIR, 'debug_poly3_time_breakdown.csv')}")
